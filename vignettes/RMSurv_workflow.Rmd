---
title: "Dynamic Treatment Regime Learning with DRMLSurv"
author: "Eric Anto"
...
output:
  ...
  word_document:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Dynamic Treatment Regime Learning with DRMLSurv}
...
d = (d_1, d_2),
$$
where $d_1(\x)$ assigns stage-1 treatment based on baseline covariates and $d_2(\x)$ assigns stage-2 treatment based on the relevant second-stage information.

The general statistical objective is to identify a treatment rule $d^\star$ that optimizes a policy value such as
$$
V(d) = \E\{Y^{d}\},
$$
where $Y^d$ is the counterfactual overall survival outcome under regime $d$.
...
```{r impute-censored-outcomes, eval = FALSE}
...
```{r match-censored-stage2, eval = FALSE}
...
```{r match-censored-stage1, eval = FALSE}
...
```{r matchpotential-2L, eval = FALSE}
...
```{r matchpotential-1L, eval = FALSE}
...
<!-- ```{r policy-summary-metrics, eval = FALSE} -->
...
The full `RMSurv` workflow can be computationally demanding. This is especially true when large SuperLearner libraries, large tuning grids, repeated cross-validation, or large datasets are used. During development and testing, it is often sensible to begin with:
