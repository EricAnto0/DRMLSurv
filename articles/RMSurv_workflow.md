...
- $`A_2 \in \{-1,1\}`$ denote the stage-2 treatment among those who
  enter stage 2,
...
d = (d_1, d_2),
```
where $`d_1(\mathbf{x})`$ assigns stage-1 treatment based on baseline
covariates and $`d_2(\mathbf{x})`$ assigns stage-2 treatment based on
the relevant second-stage information.

The general statistical objective is to identify a treatment rule
$`d^\star`$ that optimizes a policy value such as
``` math
V(d) = \mathop{\mathrm{\mathbb{E}}}\{Y^{d}\},
```
where $`Y^d`$ is the counterfactual overall survival outcome under
regime $`d`$.
...
[`rfdtr()`](https://ericanto0.github.io/RMSurv/reference/rfdtr.md) for
...
[`Drmatch()`](https://ericanto0.github.io/RMSurv/reference/Drmatch.md),
...
The full `RMSurv` workflow can be computationally demanding. This is
